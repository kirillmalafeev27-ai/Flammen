...
- `public/js/device.js` — single source of truth for the device/GPU profile, imported by both `main.js` and `game.js`. At load it probes the GPU (`WEBGL_debug_renderer_info`) and exports `profile` (pixelRatioCap, antialias, precision, releaseMultiplier, statueCullRadiusSq, anisotropy, etc.). Key job: fold weak/old Macs (integrated/Intel GPU **or** no WebGL2) into `lowPower` so they no longer get the full desktop profile. `?quality=high|low` overrides detection.
